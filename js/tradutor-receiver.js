...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // ✅ CONFIGURAÇÃO CORRETA DE IDIOMAS (cabeçalho integrado)
    ...
    // ✅ CALLBACK WebRTC (mantido do cabeçalho)
    ...
    // ✅ FUNÇÃO DE TRADUÇÃO CORRIGIDA (usando as variáveis do cabeçalho)
    async function translateText(text) {
        try {
            const response = await fetch('https://chat-tradutor.onrender.com/translate', {
                ...
                body: JSON.stringify({ 
                    text: text,
                    sourceLang: IDIOMA_ORIGEM, // ✅ CORRETO: Idioma de QUEM fala
                    targetLang: IDIOMA_DESTINO // ✅ CORRETO: Idioma para QUEM ouve
                })
            });
    ...
    // ✅ RESTANTE DO CÓDIGO ORIGINAL (funcionando perfeitamente)
    function initializeTranslator() {
        ...
        // ✅ BOTÃO MUNDO FUNCIONANDO (original)
        ...
        // ✅ BANDEIRAS FUNCIONANDO (original)
        if (languageOptions && languageOptions.length > 0) {
            languageOptions.forEach(option => {
                option.addEventListener('click', async function() {
                    const novoIdioma = this.getAttribute('data-lang');
                    IDIOMA_ORIGEM = await obterIdiomaCompleto(novoIdioma); // ✅ Usando a função do cabeçalho
        ...
        function speakText(text) {
            ...
            utterance.lang = IDIOMA_FALA; // ✅ Usando a variável do cabeçalho
            utterance.rate = 0.9;
